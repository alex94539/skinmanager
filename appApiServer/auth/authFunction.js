...
export function expireTokenByAccount(account){
        connection.query('UPDATE `login_log` SET `valid` = 0 WHERE `login_log`.`userindex` = ?',
        [account]
    );
}
...
export function addLoginLog(token, account, device, index){
    connection.query(
		'INSERT INTO `login_log` (`TID`, `Token`, `username`, `createdAt`, `Utime`, `device_ID`, `valid`, `userindex`) VALUES (NULL, ?, ?, ?, ?, ?, 1, ?)',
		[token, account, moment().format('YYYY-MM-DD hh:mm:ss'), moment().format('YYYY-MM-DD hh:mm:ss'), device, index]
	);
}
